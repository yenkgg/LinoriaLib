...
local LibraryBackgroundBlur = nil
...
-- Helper functions for mobile input
local function GetCurrentInputPosition()
    if InputService.TouchEnabled then
        local touches = InputService:GetTouchPositions()
        if #touches > 0 then
            return touches[1]
        end
    end
...
local function IsInputActive(Input)
    if Input.UserInputType == Enum.UserInputType.MouseButton1 then
        return InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
    elseif Input.UserInputType == Enum.UserInputType.Touch then
        local state = InputService:GetTouchState(Input.TouchIndex)
        return state == Enum.UserInputState.Begin or state == Enum.UserInputState.Change
    end
    return false
end
...
-- Mobile detection is intentionally touch-first. Some mobile executors report a
-- generic/unknown platform, while TouchEnabled remains reliable.
...
-- Keep the minimum size small enough for phones. The old 550px minimum could
-- prevent the Obsidian sidebar layout from fitting on narrow screens.
Library.MinSize = if Library.IsMobile then Vector2.new(300, 200) else Vector2.new(550, 300)
...
function Library:SetDPIScale(value: number) 
    ...
    DPIScale = value / 100
    Library.MinSize = (if Library.IsMobile then Vector2.new(300, 200) else Vector2.new(550, 300)) * DPIScale
end
...
function Library:AddCorner(Instance, Radius)
    ...
    local Existing = Instance:FindFirstChild("YenkggCorner")
    if Existing then
...
function Library:AddToolTip(InfoStr, DisabledInfoStr, HoverInstance)
    ...
    GiveSignal(HoverInstance.MouseEnter:Connect(function()
        ...
        local pos = GetCurrentInputPosition()
        Tooltip.Position = UDim2.fromOffset(pos.X + 15, pos.Y + 12)
        ...
        while IsHovering do
            ...
            local newPos = GetCurrentInputPosition()
            Tooltip.Position = UDim2.fromOffset(newPos.X + 15, newPos.Y + 12)
        end
...
function Library:Unload()
    ...
    if LibraryBackgroundBlur then
        pcall(function() LibraryBackgroundBlur:Destroy() end)
        LibraryBackgroundBlur = nil
    end
    ScreenGui:Destroy()
    ...
    local BaseAddonsFuncs = {}
        ...
        do
            ...
            Library:GiveSignal(KeybindsToggleRegion.InputBegan:Connect(function(Input)
                ...
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame(Input)) or Input.UserInputType == Enum.UserInputType.Touch then
        ...
        PickOuter.InputBegan:Connect(function(PickerInput)
            if PickerInput.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame(PickerInput) then
                Picking = true
            ...
            elseif PickerInput.UserInputType == Enum.UserInputType.MouseButton2 and not Library:MouseIsOverOpenedFrame(PickerInput) then
    ...
    function BaseAddonsFuncs:AddColorPicker(Idx, Info)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton2 and ContextMenu.Container.Visible then
                if not Library:MouseIsOverFrame(ContextMenu.Container, Input) and not Library:MouseIsOverFrame(DisplayFrame, Input) then
                    ContextMenu:Hide()
                end
            end
        end))
    ...
    function BaseAddonsFuncs:AddDropdown(Idx, Info)
        ...
        InputService.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                if pos.X < AbsPos.X or pos.X > AbsPos.X + AbsSize.X
                    or pos.Y < (AbsPos.Y - (20 * DPIScale) - 1) or pos.Y > AbsPos.Y + AbsSize.Y then
    ...
    function BaseGroupboxFuncs:AddSlider(Idx, Info)
        ...
        SliderInner.InputBegan:Connect(function(Input)
            ...
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame(Input)) or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                local initialPos = GetCurrentInputPosition()
                local gPos = Fill.AbsoluteSize.X
                local Diff = initialPos.X - (Fill.AbsolutePosition.X + gPos)

                while IsInputActive(Input) do
                    local currentPos = GetCurrentInputPosition()
                    local nXOffset = math.clamp(gPos + (currentPos.X - initialPos.X) + Diff, 0, Slider.MaxSize)
...
function Library:CreateWindow(...)
    ...
    if WindowInfo.Size == UDim2.fromOffset(0, 0) then
        if Library.IsMobile then
            local MobileWidth = math.clamp(ViewportSize.X - 16, 300, 700)
            local MobileHeight = math.clamp(ViewportSize.Y - 32, 220, 700)
    ...
    elseif Library.IsMobile then
        -- Clamp an explicitly supplied desktop-sized window so it cannot
        -- render mostly off-screen on phones.
        local MobileWidth = math.clamp(ViewportSize.X - 16, 300, 700)
        local MobileHeight = math.clamp(ViewportSize.Y - 32, 220, 700)
        WindowInfo.Size = UDim2.fromOffset(
            math.min(WindowInfo.Size.X.Offset > 0 and WindowInfo.Size.X.Offset or MobileWidth, MobileWidth),
            math.min(WindowInfo.Size.Y.Offset > 0 and WindowInfo.Size.Y.Offset or MobileHeight, MobileHeight)
        )
    end
    ...
    -- Legacy tab strip kept for API compatibility; the visible navigation is the Obsidian-style sidebar below.
    ...
    local Sidebar = Library:Create("Frame", {
        ...
        Size = if Library.IsMobile then UDim2.new(0, 104, 1, -16) else UDim2.new(0, 142, 1, -16);
        ZIndex = 4;
        Parent = MainSectionInner;
    })
    Library:AddCorner(Sidebar, 12)
    ...
    Library:AddToRegistry(SidebarAccent, { BackgroundColor3 = "AccentColor" })
    Library:AddCorner(SidebarAccent, 3)

    local SidebarTitle = Library:CreateLabel({
        Position = UDim2.new(0, 14, 0, 10);
        Size = UDim2.new(1, -24, 0, 24);
        Text = WindowInfo.Title or "Linoria";
        TextSize = 17;
        TextXAlignment = Enum.TextXAlignment.Left;
        ZIndex = 6;
        Parent = Sidebar;
    ...
    local SidebarTabArea = Library:Create("ScrollingFrame", {
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        Position = if Library.IsMobile then UDim2.new(0, 5, 0, 56) else UDim2.new(0, 8, 0, 56);
        Size = if Library.IsMobile then UDim2.new(1, -12, 1, -64) else UDim2.new(1, -16, 1, -64);
        CanvasSize = UDim2.new(0, 0, 0, 0);
    ...
    local SidebarListLayout = Library:Create("UIListLayout", {
        Padding = UDim.new(0, 5);
        FillDirection = Enum.FillDirection.Vertical;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = SidebarTabArea;
    })
    ...
    local TabContainer = Library:Create("Frame", {
        ...
        BorderColor3 = Library.OutlineColor;
        Position = if Library.IsMobile then UDim2.new(0, 120, 0, 8) else UDim2.new(0, 158, 0, 8);
        Size = if Library.IsMobile then UDim2.new(1, -128, 1, -16) else UDim2.new(1, -166, 1, -16);
        ZIndex = 2;
        Parent = MainSectionInner;
    })
    ...
    Library:AddCorner(TabContainer, 12)
    ...
    LibraryBackgroundBlur = Instance.new("BlurEffect")
    LibraryBackgroundBlur.Name = "YenkggLinoriaBlur"
    LibraryBackgroundBlur.Size = 0
    LibraryBackgroundBlur.Parent = Lighting
    ...
    function Window:AddTab(Name, Icon)
        ...
        local TabButton = Library:Create("TextButton", {
            ...
            BorderColor3 = Library.OutlineColor;
            Size = if Library.IsMobile then UDim2.new(1, 0, 0, 36) else UDim2.new(1, 0, 0, 38);
            Text = "";
            ZIndex = 7;
            Parent = SidebarTabArea;
        })
        Library:AddCorner(TabButton, 9)
        ...
        local IconMap = {
            home = "⌂", settings = "⚙", visuals = "◉", combat = "✦", player = "♙", misc = "◆", config = "▣", theme = "◈", themes = "◈", info = "ⓘ",
        }
        local TabIcon = Icon or IconMap[string.lower(tostring(Name))] or "•"
        local TabIconLabel = Library:CreateLabel({
            Position = UDim2.new(0, 6, 0, 0);
            Size = if Library.IsMobile then UDim2.fromOffset(20, 36) else UDim2.fromOffset(24, 38);
            Text = TabIcon;
            TextSize = 16;
            TextXAlignment = Enum.TextXAlignment.Center;
            ZIndex = 8;
...
do
        ...
        function Tab:ShowTab()
            ...
            if Library.PlayTabSound then
                task.spawn(Library.PlayTabSound)
            end
    ...
    function Window:Toggle(Toggling)
        ...
        if LibraryBackgroundBlur then
            local TargetBlur = Toggled and 18 or 0
            TweenService:Create(LibraryBackgroundBlur, TweenInfo.new(FadeTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = TargetBlur
            }):Play()
        end
    ...
    if Library.IsMobile then
        ...
        Library:AddCorner(ToggleUIOuter, 10)
        Library:AddToRegistry(ToggleUIOuter, { BackgroundColor3 = "BackgroundColor" })
        ...
        Library:AddCorner(ToggleUIInner, 9)
        Library:AddCorner(ToggleUIInnerFrame, 8)
        Library:AddToRegistry(ToggleUIInnerFrame, { BackgroundColor3 = "MainColor" })
        ...
        Library:AddCorner(LockUIOuter, 10)
        Library:AddToRegistry(LockUIOuter, { BackgroundColor3 = "BackgroundColor" })
        ...
        Library:AddCorner(LockUIInner, 9)
        Library:AddCorner(LockUIInnerFrame, 8)
        Library:AddToRegistry(LockUIInnerFrame, { BackgroundColor3 = "MainColor" })
